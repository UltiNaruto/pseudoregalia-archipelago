	...
	void Engine::SpawnCollectibles() {
		...
		for (const auto& [id, collectible] : collectible_map) {
			...
			if (id == timeTrial)
			{
				...
				bestTime = *timeTrial_obj->GetPropertyByName(STR("bestTime"))->ContainerPtrToValuePtr<double>(timeTrial_obj);
				goalTimes = *timeTrial_obj->GetPropertyByName(STR("goalTimes"))->ContainerPtrToValuePtr<TArray<double>>(timeTrial_obj);
